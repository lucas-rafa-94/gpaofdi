                                    ...
                                    steps {
                                         sh """ build=correios-test-`date +'%Y%m%d%H%M%s'` && \
                                                curl --insecure -X POST \
                                                  ...
                                                  -d '{"deployment_id":"`date +'%Y%m%d%H%M%s'`","deployment_name":"`date +'%Y%m%d%H%M%s'`","desired_state":1,"placement":{"pool_id":"default"},"quantities":{"nginx":1},"stack":{"service_id":"correios-test","service_name":"correios-test","subtype":"service"}}' """
                                    }
                                }
    }
}